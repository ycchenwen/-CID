...
data_in = pd.read_excel('related account module.xlsx',sheetname='根据POS排列匹配覆盖')
la = list(un_fg['st_address'].unique())
filtered_fg = all_fg[True ^ all_fg['st_address'].isin(la)].copy()
filtered_fg_group = filtered_fg.groupby(['pos'])
lst = []
for name, group in filtered_fg_group:
    ...
    lst.append(s_temp)
#  列中含有数值项时不能str拼接
filtered_fg['POS排列'] = filtered_fg['pos']+'-'+pd.concat(lst).map(str)
data_out = pd.merge(data_in,filtered_fg,on='POS排列',how='left')
data_out.to_excel('FTTH账号根据POS排列匹配覆盖地址结果.xlsx',index=False)
